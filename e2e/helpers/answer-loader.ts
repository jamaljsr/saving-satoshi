...
export function getAnswerFromFile(
  ...
  let filePath: string
  if (language) {
    // Code challenge: e2e/answers/chapter{N}/{language}/{lesson}-answer.{js|py}
    ...
    // Non-code challenge: e2e/answers/chapter{N}/{lesson}-answer.txt
    filePath = path.join(
      ANSWERS_DIR,
      `chapter${chapter}`,
      `${lesson}-answer.txt`
 ...
 * JSON files are stored as: e2e/answers/chapter{N}/{lesson}.json
 */
 ...
 * JSON files are stored as: e2e/answers/chapter{N}/{lesson}-answer.json
 */
